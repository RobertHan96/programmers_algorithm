...
def solution(A, B):
    answer = 0
    # A는 오름차순, B는 내림차순으로 정렬하면 문제에서 원하는 최솟값의 기준이 되므로
    # A, B를 정렬하고 시작한 다음 반복문을 1번만 사용하면 해결 가능
    A.sort()
    B.sort(reverse=True)
...
def bestSolution(A, B):
    # zip, 리스트 컴프리헨션으로 한줄로도 해결 가능
    # zip함수 = 여러 배열에서 같은 인덱스값만 가져와서 연산 수행 가능
